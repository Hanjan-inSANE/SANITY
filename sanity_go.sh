...
def patch(path, old, new, marker, label):
    ...
    except FileNotFoundError: print("  [파일없음]",label); return
...
patch("sanity_defender/main.py",
  ...
  'gateway_url=p["gateway_url"], ', "defender gateway_url")
...
if echo "$resp" | grep -q '"content"'; then
  ...
  echo "  !! '$WANT_MODEL' 없음 → 기존 유지. 응답: $(echo "$resp" | head -c 160)"
  echo "  사용가능 gemma:"; curl -s -m 20 https://ollama.com/v1/models -H "Authorization: Bearer $OK_KEY" | python3 -c 'import sys,json
try:
 [print("    ",m["id"]) for m in json.load(sys.stdin).get("data",[]) if "gemma" in m["id"].lower()]
except Exception as e: print("   목록실패",e)' || true
fi
...
for i in $(seq 1 40); do
  ...
  echo "  health try $i: $code"; [ "$code" = "200" ] && { ok=1; break; }; sleep 3
done
( cd deploy && docker-compose ps )
echo "  --- SM 부팅 로그 ---"; ( cd deploy && docker-compose logs --tail=15 scenario-manager )
[ -z "$ok" ] && { echo "!! gateway 실패"; ( cd deploy && docker-compose logs --tail=30 gateway ); exit 1; }

echo "############ [6] 실행 (뷰어 http://localhost:8090) ############"
